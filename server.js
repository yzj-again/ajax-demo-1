...
var server = http.createServer(function (request, response) {
    ...
    if (path === '/index.html') {
        ...
        const result = arr.map(item=>`<li>${item.id}</li>`).join('')//连接li
        // console.log(typeof fs.readFileSync('public/index.html')) object
        string=string.replace('{{ page1 }}',`<ul id="xxx">${result}</ul>`)
        response.write(string)//在body里请求main.js//文件变成字符串,用nodejs一个模块
        response.end()
